...
class TicTacToeHttpServer:
    def __init__(self, host='localhost', port=55556):
        ...
        # Game state
        self.games = {}  # game_id: {'board': [], 'players': [], 'current_turn': 0, 'status': 'waiting/playing/finished', 'winner': None}
    ...
    def handle_request(self, client_socket):
        try:
            ...
            response = self.create_response(500, "Internal Server Error", {"status": "ERROR", "message": str(e)})
        finally:
    ...
    def process_request(self, request_data):
        ...
        try:
            ...
            return self.create_response(400, "Bad Request", {"status": "ERROR", "message": "Malformed request line"})
        ...
        with self.lock:
            if method == 'POST' and path.startswith('/player/'):
            ...
            elif method == 'POST' and path == '/game/leave':
                data = json.loads(body)
                ...
                if player_id in self.players:
                    ...
                    if game_id and game_id in self.games:
                        ...
                        result = {"status": "OK", "message": "Player left the game"}
                    else:
                        result = {"status": "ERROR", "message": "Player not in a game"}
                else:
                    result = {"status": "ERROR", "message": "Invalid player ID"}
            else:
                return self.create_response(404, "Not Found", {"status": "ERROR", "message": "Endpoint not found"})
    ...
    # --- Game Logic Methods (mostly unchanged, but with locking) ---
    ...
    def create_game(self, player_id):
        if player_id not in self.players or self.players[player_id]['game_id']:
    ...
    def join_game(self, player_id, game_id):
        ...
        if len(game['players']) == 1:
            ...
            return {"status": "OK", "message": "Joined game", "game_id": game_id}
        else:
    ...
    def get_game_state(self, player_id):
        ...
        game = self.games[game_id]
        current_player = game['players'][game['current_turn_idx']] if game['status'] == 'playing' else None
        
        return {
            "status": "OK",
            "game_state": {
                "board": game['board'], "game_status": game['status'],
                "current_turn": current_player, "winner": game['winner'],
                "your_symbol": self.players[player_id]['symbol'],
                "players": game['players'], "symbols": game['symbols']
            }
        }
    ...
    def make_move(self, player_id, row, col):
        ...
        game['board'][row][col] = self.players[player_id]['symbol']
        
        winner = self.check_winner(game['board'])
        if winner:
            game['status'] = 'finished'
            game['winner'] = self.players[player_id]['symbol']
            # Clear game_id for both players when game finishes
        ...
        return {"status": "OK", "message": "Move made", **self.get_game_state(player_id)}

    def check_winner(self, board):
        # (check logic is unchanged)
        for i in range(3):
